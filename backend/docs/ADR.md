...
## BE-ADR-046：R6 以统一不可变执行账本连接真实 MCP 与本地扩展能力

- 状态：已采用；R6 前后端实现、助手隔离返修、自动化和受控页面证据已于 2026-09-08 通过总体协调窗口正式验收
- 决定：新增迁移 `029`、统一能力目录与执行/恢复 API。R1 的 `model_api` 执行只投影既有事实，不触发第二次模型调用；`local_tool` 只允许内置确定性实现；Skill 是不可变步骤编排，Plugin 是不可执行代码的本地清单与生命周期映射。
- MCP：只实现官方 `2026-07-28` Streamable HTTP 单次 POST 合同，支持 JSON 与请求作用域 SSE、严格 JSON Schema 2020-12 子集及 `x-mcp-header`。生产目标必须是无凭据、无 query/fragment 的公共 HTTPS，执行 DNS 全答案检查、固定连接地址、禁止重定向并限制超时和正文；loopback HTTP 只能由测试显式注入。
- 安全与恢复：安装/发现/生命周期及执行继续经过 R2 会话、当前助手、Permission、Security/Confirmation 和 Audit。明确 429/5xx 是 `retryable`，但只能由新的明确 recovery 操作重试；发送后断线/超时是 `outcome_unknown`，查询、重启和同键重放均不得产生第二次外部调用。
- 归属：R2 Provider/Model 是 owner 级账户配置；R6 安装定义/版本、MCP 发现、配置与生命周期操作、execution/attempt/result/usage 则全部绑定 `owner + current assistant`。同一 owner 的两个 assistant 可独立安装同名能力和复用操作键，但不得共享目录、发现、依赖、生命周期或执行历史；跨 owner 同样 fail closed。
- 边界：不执行插件脚本、Shell、文件或任意网络代码，不修改 Subject Runtime Port、Engine 合同或历史 V1—V5。自动化只使用临时 SQLite 与随机 loopback MCP；真实 Engine、真实 MCP 服务、真实凭据、业务公网和费用均未执行。
- 合同：[`R6_UNIFIED_CAPABILITY_EXECUTION_CONTRACT.md`](R6_UNIFIED_CAPABILITY_EXECUTION_CONTRACT.md)

## BE-ADR-045：R5 本地长期记忆使用独立不可变账本并作为 R4 的精确来源

- 日期：2026-09-08
- 状态：已采用；合同、迁移、后端/前端实现、隔离复验与受控页面证据已完成，2026-09-08 已由总体协调窗口正式验收
...
- 日期：2026-09-07
- 状态：已采用；实现、验收返修、隔离自动化和受控页面证据完成，2026-09-08 已由总体协调窗口正式验收
- 归属与权威：身份只来自 R2 服务端会话，助手/会话/分支只来自 R2/R3 持久化选择。R4 属于 Vio 独立模式，复用 R1/R3 唯一 turn、execution、Provider attempt、result、usage 和发布边界，不创建第二条模型执行链。
- 固定装配：来源顺序冻结为系统规则、当前助手设定、可选已验证通用运行时投影、未解决 Event、近期原始 MessageVersion、R5 memory 空槽、当前用户消息。来源必须通过 owner/assistant/conversation/branch、可见性和权限检查；当前消息与强制规则不能排除、折叠或静默丢弃。
- 预算与折叠：上下文上限来自明确 model-limit port 或保守 Vio policy，先保留完整输出预算，再用确定性 UTF-8 上界估算裁剪。长历史只能形成引用精确来源的不可变结构化 summary；失败时原文可装入才允许显式 fallback，否则在 Provider 前停止并等待受控恢复。
- 持久化与恢复：迁移 `027` 保存对话设置、turn 控制、计划/来源、summary/attempt、锁定 assembly、恢复 action，并把 `context_snapshot_hash` 绑定到原 standalone execution。同 turn 重放/重启复用同一快照；Provider 已可能发送的 `outcome_unknown` 不重组上下文、不盲重试。
- 验收返修：跨窗口最终选择以当前用户消息做相关性排序，preview 明确为 provisional；只选仍可读取且来源 hash 一致的最新 ready summary。失效排除项保留审计但不作用于新装配，长 preview 先模拟折叠/裁剪再决定可发送。summary builder 输入不可变且输出严格校验；failed candidate、locked source、summary/assembly relational facts 与快照在恢复时原子一致。
- 外部边界：None/no runtime 是正常状态；可选投影必须已经通过通用 Subject Runtime Port 边界验证，R4 不执行外部运行时、不伪造 SubjectState/revision/Event/StateMutation。R5 记忆明确未实现，历史 Continuity 专用链路不作为 R4 数据源。
- 验证边界：自动化只使用临时 SQLite、隔离 owner/assistant/conversation、确定性预算/summary test port 与随机 loopback Provider。真实 Engine、真实供应商、真实凭据、业务公网、部署和收费均未执行。
...
- Provider 安全：正式 `openai_compatible` 请求只允许 HTTPS；DNS 解析后拒绝任一危险地址，选择安全 IPv4 并固定实际连接地址，禁止重定向并设置连接、响应和正文上限。IPv6-only 明确不支持；loopback HTTP 只允许显式测试装配。凭据仅在调用瞬间解密，不进入响应、账本、日志或测试快照。
- 旧入口收口：生产个人会话拒绝 legacy Conversation、Message、Continuity Turn 和 `state_update` 写入口，返回 `PERSONAL_CHAT_ROUTE_REQUIRED`；历史数据、只读能力与显式 test-support 兼容证据保留，不删除也不冒充 R1 结果。
- Engine 边界：R1 不构造 Continuity V1 请求，不进入 V2—V5，不读取 Binding、SubjectState 或 Engine result，也不探测、读取、启动或修改 Engine。历史 V1—V5、F1、S4/S4-Live 继续作为 Continuity Engine Adapter 专用证据保留，不是 R1 生产路径的依赖或验收替代。
- 交付边界：总体协调窗口于 2026-09-05 正式验收 R1 通过。真实供应商、手机实机、云部署、跨设备同步和 R3 多会话均未执行；受控 loopback 测试不是外部供应商验收。
...
- 现行状态（2026-09-08）：上项保留当时施工事实；R0、R2、R1、R3、R4、R5 与 R6 均已正式验收。R7 未开始。
...
- 现行状态（2026-09-08）：原状态行保留 R0 当时进度；R0、R2、R1、R3、R4、R5 与 R6 均已正式验收。R7 未开始。
