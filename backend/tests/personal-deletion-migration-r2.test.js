...
test('024 fresh and 001-023 upgrade retain old identities and ordinary immutable guards',()=>{
  ...
  try {
   ...
   assert.equal(upgraded.connection.prepare("SELECT count(*) n FROM sqlite_master WHERE type='trigger' AND sql LIKE '%WHEN vio_owner_deletion_authorized%'").get().n,59);
