...
test('024 fresh and 001-023 upgrade retain old identities and ordinary immutable guards',()=>{
 ...
 try {
  const oldPath=join(root,'old');cpSync(resolve('migrations'),oldPath,{recursive:true});rmSync(join(oldPath,migration));
  ...
  assert.equal(upgraded.connection.prepare("SELECT count(*) n FROM sqlite_master WHERE type='trigger' AND sql LIKE '%WHEN vio_owner_deletion_authorized%'").get().n,24);
  assert.equal(upgraded.connection.prepare('SELECT count(*) n FROM personal_deletion_tasks').get().n,0);
  assert.deepEqual(upgraded.connection.prepare('PRAGMA foreign_key_check').all(),[]);upgraded.close();
