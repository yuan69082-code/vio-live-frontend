...
test('024 fresh and 001-023 upgrade retain old identities and ordinary immutable guards',()=>{
  ...
  try {
  const oldPath=join(root,'old');cpSync(resolve('migrations'),oldPath,{recursive:true});rmSync(join(oldPath,migration));rmSync(join(oldPath,'025_create_standalone_chat_ledger.sql'));rmSync(join(oldPath,'026_create_personal_multi_conversation.sql'));rmSync(join(oldPath,'027_create_context_assembly_ledger.sql'));rmSync(join(oldPath,'028_create_local_long_term_memory.sql'));rmSync(join(oldPath,'029_create_unified_capability_execution.sql'));
  ...
  try {
   ...
   assert.equal(upgraded.connection.prepare("SELECT count(*) n FROM sqlite_master WHERE type='trigger' AND sql LIKE '%WHEN vio_owner_deletion_authorized%'").get().n,68);
