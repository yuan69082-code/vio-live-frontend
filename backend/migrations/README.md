...
- `026_create_personal_multi_conversation.sql` 新增 R3 会话目录/当前选择、分支及分支消息投影、消息版本事实、重新生成、受管附件、导出、操作与事件账本。它把每条 R1 默认会话精确登记为相同 owner/assistant 下的第一个 R3 会话和 root branch，不复制或改写 Conversation、Message/MessageVersion、R1 turn/execution/attempt/usage/cost/result 或 Event。
- `026` 使用复合 owner/assistant/conversation 外键、当前选择和活动状态约束、每会话单活动 turn，以及 operation/idempotency 不可变保护。受管附件正文位于仓库外受控根，数据库只保存 hash/大小/相对对象键并登记到 R2 `personal_managed_copies`；删除只在精确引用和 owner 范围内进行。fresh、`001–025` upgrade、迁移失败整体回滚、外键与旧 R1 投影由 R3 迁移专项覆盖，不修改 `001–025`。
- `027_create_context_assembly.sql` 新增 R4 对话 Context 设置、turn 控制、assembly/source、结构化 summary/attempt 与 recovery action；唯一 snapshot hash 绑定原 standalone execution，来源引用、summary 和预算决策不可变，不复制 R1/R3 turn、Provider attempt、result、usage 或 MessageVersion。
- `028_create_local_memory.sql` 新增 R5 owner/assistant 范围的记忆、不可变版本、精确来源、检索/Context 使用、归档/恢复、导入/导出与等待期删除事实；只把精确 version/source hash 投影到 R4 `long_term_memory` 槽，不改写已锁定 turn，也不冒充 R11 备份或云同步。
- `029_create_unified_capability_execution.sql` 新增 R6 统一能力目录、配置版本、不可变执行/attempt/result、usage/cost、审计投影和恢复 action 账本。能力定义/版本、发现快照、配置与生命周期操作、execution 及其完整子事实均由 `owner + current assistant` 复合归属保护；同一 owner 的两个 assistant 可独立安装同名能力并复用 operation/idempotency key，不共享目录、发现、依赖、生命周期或执行历史。每个 owner/assistant/幂等键只建立一个 execution；每次 attempt 独立保存状态与外部发送边界，最多一个成功 result。明确 retryable 只能经显式恢复和重新门控增加 attempt；`outcome_unknown` 保持 fail closed。R1 模型 execution 通过稳定映射投影，不复制 Provider 调用或 usage。fresh、`001–028` upgrade、故障整体回滚、外键、部分唯一索引、不可变触发器、owner 删除及跨 owner/assistant 隔离由 R6 迁移专项覆盖，不修改 `001–028`。
