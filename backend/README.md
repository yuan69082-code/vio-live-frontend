...
首次正式 S4-Live 于 2026-08-14 在 `C:\VioS4\first-001` 完成，使用 `purpose=s4-live-acceptance`、`identityClass=disposable_test`、`promotionAllowed=false` 和 Alibaba Cloud Model Studio OpenAI-compatible Provider。`qwen-flash-2025-07-28` 恰好执行一次并报告 177/9/186 Token，CapabilityResult 为 `SUCCEEDED`、首次回传 HTTP 200、result outbox 为 completed；Turn 为 completed，最终主体表达“Vio首次真实试聊连接成功。”来自 Engine `FirstRoundSuccessResult`。Engine 保持 `changed=false / revision=0`，未创建内部 Event/StateMutation；无 capability incident、outcome_unknown、重复 execution/result 或重复 Message。验收后 5173/8787/8766 均已停止监听，官方 cleanup 整根删除沙箱。Vio 保存 `cost_status=not_reported`；验收时供应商费用概览显示 ¥0，但账单可能延迟，不能表述为永久最终费用已确认。
...
Vio 的通用运行时边界入口是 [`Subject Runtime Port v1`](docs/SUBJECT_RUNTIME_PORT_V1.md)。现有 [`Continuity Integration Contract v1.1`](../docs/后端/14-continuity-engine连接契约v1.1.md) 及 [`14c`](../docs/后端/14c-Engine-Contract-Final-Read-Only-Short-Confirmation-v1.md) 只登记为 Continuity Engine Adapter 专用合同；在该适配器范围内继续固定以下边界：
...
- API Key 原值继续禁止进入请求、SQLite、响应、日志和 Git；Provider 可通过高风险确认绑定/轮换 `env:VIO_MODEL_API_KEY_*` secretRef，实际密钥只在调用瞬间解析且外部响应只显示配置状态
...
| `VIO_CONTINUITY_ENGINE_ENABLED` | `false` | 显式启用正式本机 Engine transport |
| `VIO_CONTINUITY_ENGINE_BASE_URL` | `http://127.0.0.1:8766` | 只接受无凭据的本机 HTTP origin；启用时必须显式提供 |
| `VIO_CONTINUITY_ENGINE_TOKEN` | 无 | 启用时必填的 Bearer service token，不保存、不回显 |
| `VIO_CONTINUITY_ENGINE_CONNECT_TIMEOUT_MS` | `2000` | 连接超时（毫秒） |
| `VIO_CONTINUITY_ENGINE_RESPONSE_TIMEOUT_MS` | `20000` | 响应超时（毫秒） |
| `VIO_CONTINUITY_ENGINE_MAX_RESPONSE_BYTES` | `1048576` | 响应体上限（字节） |
| `VIO_MODEL_PROVIDER_CONNECT_TIMEOUT_MS` | `5000` | Provider 连接超时（毫秒） |
| `VIO_MODEL_PROVIDER_RESPONSE_TIMEOUT_MS` | `60000` | Provider 响应超时（毫秒） |
| `VIO_MODEL_PROVIDER_MAX_REQUEST_BYTES` | `1048576` | Provider 请求体上限（字节） |
| `VIO_MODEL_PROVIDER_MAX_RESPONSE_BYTES` | `2097152` | Provider 响应体上限（字节） |
| `VIO_MODEL_API_KEY_*` | 无 | 由已绑定的 `env:VIO_MODEL_API_KEY_*` secretRef 在调用瞬间解析；值不得写入配置文件、数据库或 Git |
...
| `PATCH` | `/api/v1/users/:userId/api-providers/:providerId/credential-reference` | 经高风险确认绑定或轮换受限环境变量 secretRef；不接受密钥原值 |
...
│  ├─ integrations/secrets/  # secretRef 校验与调用瞬间的环境凭据解析
│  ├─ integrations/model-providers/ # openai_compatible 非流式 HTTP adapter
...
│  ├─ modules/continuity-integration/ # V1 请求、V2 投影、V3 交付与 V4 Capability 闭环
...
平台后端与 continuity-engine 保持平行。Vio 负责用户、助手、会话、平台数据、权限安全、私域安全存储、Token 和外部能力安全通道；continuity-engine 负责 Wake、Perception、Thinking、Learning、Action、Revision、最终认知 Context、唯一权威 SubjectState 和最终主体表达。V1–V4 及 S2/S3/S4 已验证机器契约、正式本机交付和 Capability；V5 在固定本地 Profile 下把公共轮次接入该链路，并只从 Engine 稳定结果发布主体 Message；F1 已让现有对话页消费 V5 公共 API。S4-Live 又以一次可销毁测试身份验证了真实 OpenAI-compatible Provider，但没有改变双方数据库和权威边界：Vio 不创建引擎 Event/StateMutation，也不改写 legacy SubjectState。真实登录、系统语音、外部存储、机器人、支付/银行、健康设备和生产部署仍未实现。

V5 固定本地 Profile 公共轮次 API、F1 前端接线、L1 安全准备入口与首次 S4-Live 真实供应商验收均已完成。本机个人日常使用化、通用正式身份/Binding、生产认证、多租户、加密密钥存储、备份和部署仍未开始，当前只允许本机受控使用，不得公开部署。
