# 06｜模型路由、密钥与 Token 管理

## 状态

- 文档状态：平台后端 6 的模型/API 路由配置基础已实现，真实连接仍在规划
- 真实模型连接：未完成
- 本地配置与路由：已完成 Provider/Model 持久化、六类任务默认/备用规则和确定性本地选择
- Token Budget、Wake、主动提示和后台策略已形成开发期配置/安全准备闭环；真实语音、模型调用和供应商计量仍未接入
- 当前限制：不指定具体模型 SDK、计费实现或密钥服务产品

## 目标

允许不同主体和不同任务使用不同模型或服务，同时保持主体状态独立、密钥安全、费用可见，并避免无意义的后台调用和无限 Token 消耗。

## 支持的服务来源

- 官方 API
- 明确标记的第三方中转服务
- 用户自建服务
- 本地模型

中转服务必须说明数据可能经过第三方服务器，不能伪装为官方服务。

## 任务路由范围

| 任务类型 | 初始可配置内容 |
| --- | --- |
| 聊天与长文本 | 默认模型、备用模型、上下文、温度和费用限制 |
| 看图与生图 | 图片理解或生成服务、尺寸、质量和风格 |
| 视频理解与生成 | 上传、抽帧、时长、质量和成本 |
| 语音识别与合成 | 识别服务、声音、语速和情绪 |
| 联网搜索 | 搜索服务、引用规则和隐私要求 |

## 模型服务配置

每项模型/API 配置至少需要表达：

- 显示名称
- 服务商和服务来源
- Base URL
- 模型名
- 接口格式
- 连接测试状态
- 默认任务和适用范围
- 费用说明
- 备用模型

本阶段已确定下面“当前基础实现”的开发期字段与接口；供应商专属参数、真实调用和生产密钥方案仍不在本文件中固化。

### 当前基础实现

- Provider 支持 `openai`、`claude`、`glm`、`custom` 四种配置类型，只表示本地目录分类，不代表已接入对应服务。
- Provider 保存显示名称、类型、Base URL、接口格式、`enabled`/`disabled` 状态、测试状态和创建/更新时间。接口格式支持 `openai_compatible`、`anthropic_messages`、`glm_compatible`、`custom_http`。
- Model 归属于 Provider，保存模型名称、模型类型、费用说明、测试状态和 `chat`、`long_text`、`vision`、`image`、`video`、`audio`、`search`、`embedding` 能力标签。
- 可路由任务严格为 `chat`、`long_text`、`image`、`video`、`audio`、`search`；`vision` 与 `embedding` 当前只作为目录查询能力。
- Model Routing Rule 按用户与任务唯一保存默认模型、可选备用模型和 `enabled`/`disabled` 状态。默认和备用必须属于同一用户、支持该任务且不能相同。
- Base URL 只保存配置，不用于当前网络请求；包含用户名、密码、片段或凭据查询参数时会被拒绝。
- Provider/Model 测试状态当前固定初始化为 `not_tested`，不接受客户端伪造；费用说明只是配置文本，不执行计费。

## 路由原则

- 用户可以手动选择模型。
- 后续允许系统按任务选择模型，但第一版不要求完整自动费用优化。
- 主模型不可用时，可按已配置规则使用备用模型。
- 更换模型不改变 `subject_id`、主体状态、长期设定或记忆归属。
- 建议模型和实际调用分开，调用仍受权限与费用限制。

当前 Router 遵循以下确定性规则：启用的显式规则优先选择 Provider 已启用的默认模型；默认 Provider 停用时，选择同规则中 Provider 已启用的备用模型；显式规则的模型均不可用时明确返回失败，不绕过用户规则。规则不存在或已停用时，才按稳定创建顺序返回首个能力匹配且 Provider 启用的模型。这里的备用选择只基于配置状态，不包含网络探测、请求重试或运行期故障切换。

Router 响应固定标记模型和外部 API 未调用。模型选择不读取或改写 Context、SubjectState、Assistant Global Settings，不装配提示词、不生成回复，也不连接 MCP、Tool 或 continuity-engine。

## 密钥管理

- API Key 只能进入服务器密钥库或设备安全存储。
- 密钥不得写入 GitHub、普通日志、事件正文或模型上下文。
- 前端只能看到脱敏状态，不应取回密钥原文。
- 外部服务撤销后，相关密钥和调用能力必须失效。
- 具体加密、轮换和密钥库方案等待安全 ADR 决定。

当前 `api_key_secret_ref` 只是必须为 `NULL` 的数据库占位。后端提供安全存储端口，但当前实现只返回 `storage=secure_store_required` 与 `writeSupported=false`，没有保存方法。Provider 接口拒绝 Key、Token、Secret、凭据或凭据引用输入，响应只返回 `not_configured`，因此本阶段不能配置或使用任何真实 API Key。

## Token 与资源原则

- 打开软件不调用模型。
- 普通页面操作优先形成软件事件。
- 只有需要理解、回应或主动唤醒时才调用模型。
- 模型优先读取当前摘要、近期事件和相关记忆。
- 其他内容按需检索，避免上下文无限增长。
- 当前可按用户/主体保存每日与会话预算，超额策略为阻止、要求确认或延后。
- 当前 Token 使用账本只接受显式上报的输入/输出计量，固定标记平台未调用模型、未计费；实际供应商统计和费用展示仍在真实模型接入后实现。

### 当前 Token 控制基础

- 每个用户/主体最多一个活动预算，`sessionTokenLimit` 不得超过 `dailyTokenLimit`。
- 预算检查按 UTC 日与不透明预算会话 ID 汇总已记录用量；检查本身不预留、不写入、不消耗 Token。
- `block` 立即返回预算拒绝；`defer` 返回延后；`require_confirmation` 使用预算 ID 作为精确 `proactive_interaction` 资源进入高风险确认。
- 使用记录可以引用同用户 Model，但不读取 Provider Key、不调用 Router、不生成回复。`explicit_api_input` 只说明来源，不构成真实供应商账单证明。

## 语音与主动唤醒边界

- 网页/PWA 第一版只支持按钮和前台语音。
- 桌面、锁屏和后台唤醒属于原生 App 或设备伴侣服务。
- 咳嗽、呼救声或自定义声音优先在本机判断，只提交必要事件。
- 触发后可以先询问用户是否需要帮助，不默认直接执行操作。

### 当前 Wake 与主动提示基础

- Wake Registry 支持 `voice`、`desktop`、`schedule`、`event` 四类，保存结构化触发条件、启停和应用内授权。
- `voice` 仅是配置分类；响应固定标记麦克风、系统唤醒和触发执行均未发生。
- 主动提示规则必须引用软件 Event 类型，保存紧急、重要、普通、静默四级优先级；静默规则只形成抑制记录。
- `requiresConfirmation=true` 把安全最低风险抬高为 `high`，不能由长期允许或短时会话授权跳过逐次确认。
- 后台策略保存 `idle` / `active`、后台开关、频率上限、允许 Wake 类型和安静时段；当前没有调度器，频率字段不触发后台任务。
- Wake、提示和需确认的 Token 超额统一使用 `proactive_interaction` Permission → Security Policy → Confirmation，所有结果不投递消息、不调用模型。

## 第一版验收方向

- 至少一个真实模型可被手动选择和安全调用。
- 密钥不会出现在前端、日志、事件或 Git 仓库中。
- 上下文装配与 Token 使用范围可解释。
- 模型切换不影响主体连续状态。
- 打开应用和普通浏览不会自动消耗模型 Token。

当前阶段满足配置目录、显式默认/备用规则、主动交互安全准备与本地 Token 预算基础，不满足“至少一个真实模型可被安全调用”的最终验收方向。
