...
R0-A 已冻结 Vio `Subject Runtime Port v1` 后端合同；R0-B 已默认装配 None Adapter，并提供 `GET /api/v1/subject-runtime/status` 与 `/health.subjectRuntime` 的通用只读状态。Vio Core 不以任何外部主体运行时为启动条件，`none / disconnected / not_configured` 是合法状态，平台仍为 `available`。Continuity Engine 在通用端口中仍仅登记为未接线的可选适配器，第三方适配器只保留受校验注入口。当前聊天业务继续使用既有接线，尚未切换到该端口。R0 总阶段仍未完成，R1 尚未开始。
...
- 尚未实现：真实供应商 live smoke、F1 前端真实回复、通用 Binding、生产认证、多租户和部署。固定本地公共 API 完成不表示产品已经可公开使用。
- 分支、消息删除、窗口重置、真实摘要生成、语义检索、真实供应商 Token 计量/计费与 live smoke、私域披露/删除、真实导出文件/下载/外部存储、备份恢复、生活记录删除、自动提醒、支付/银行、健康设备、真实语音/系统唤醒、后台调度、消息投递、正式数据库、公共 Capability 产品入口、通用对话 Profile 与前端接线、真实 MCP、插件安装、Skill/Tool 执行、真实设备/机器人连接控制、载体迁移、厂商 API、其他外部执行和公开运行能力仍处于规划或待接入状态。
