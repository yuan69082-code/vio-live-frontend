...
本目录记录稳定规划。仓库已完成可运行平台后端、开发数据库、User/User Space/Subject/Dashboard、当前助手与数据隔离检查、AI Assistant Global Settings、对话/摘要/状态/Context、Event、模型路由、Permission/Security、扩展与设备注册、AI Private Space，以及管账、月历、身体管理和本地记忆数据基础；前端已建立独立 API 客户端和真实健康握手。这不代表真实认证、页面数据迁移、正式数据库、事件消费、真实摘要/AI 建议生成、模型连接/测试、API Key、continuity-engine、支付/银行、私域披露决策、真实 MCP/Tool、真实设备或穿戴健康数据已经完成。
