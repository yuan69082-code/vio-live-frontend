...
本目录记录稳定规划。仓库已于 2026-07-25 建立 `backend/` 工程设计骨架，但这不代表已经初始化可运行后端、数据库、API、模型连接、MCP、设备适配或外部执行能力。
...
- 已建立独立 `backend/` 设计目录、工程 README、后端 ADR、开发日志和第一阶段实施路线。
- 后端目录当前不包含运行依赖、服务入口、数据库迁移或真实集成。
- 已完成核心数据库逻辑模型和数据关系设计，但尚未选择或连接真实数据库。
