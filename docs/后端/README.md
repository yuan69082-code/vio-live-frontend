...
本目录记录稳定规划。仓库已于 2026-07-25 完成首个可运行平台后端、开发数据库及 User/Subject 闭环；这不代表真实认证、正式数据库、完整 API、模型连接、MCP、设备适配或外部执行能力已经完成。
...
- 后端可以独立启动，并提供健康检查与 User/Subject 最小闭环。
- 已完成核心逻辑数据模型，并使用开发期 SQLite 实现 `users`、`subjects` 和迁移记录。
