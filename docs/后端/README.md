...
R0-A/B/C 及 R0 整阶段已验收并推送；状态卡本身不证明独立聊天完成。R2 个人所有者访问、首次设置、同用户多助手、会话撤销、访问审计/诊断、Provider/Model、加密凭据、认证连接检查及 7/14/30 天政策下的受控账户删除，以及 R1 真实个人会话/当前助手范围的独立聊天合同、迁移 `025` 与 Vio 自有执行/恢复边界，均已于 **2026-09-05 正式验收通过**。R3 的多会话目录、消息版本/分支、受控再生成、附件、导出、迁移 `026`、前后端实现和受控页面闭环已于 **2026-09-06 正式验收通过**。R4 的上下文合同、迁移 `027`、独立聊天锁定快照、来源/折叠/预算、隔离回归与受控页面闭环，R5 的本地长期记忆闭环，以及 R6 的迁移 `029`、owner/current-assistant 隔离统一能力目录/执行/恢复、确定性本地 Tool、不可变 Skill/Plugin、MCP `2026-07-28` 和 R1 模型事实投影，均已于 **2026-09-08 正式验收通过**。邮箱、Google 和公开注册继续暂缓。

已确认产品顺序：R0 → R2 → R1 → R3 至 R13，编号不变；R0、R2、R1 与 R3 已正式验收，后续阶段须单独授权。R2 完成个人访问和同一用户多个助手的创建、列表、选择与普通切换；R3 完成每个助手多个会话及隔离。R8 完成公共机制及截至 R7 已完成模块的前端接线；R9—R11 各自完成对应前后端真实联调，R13 全量复验；保留工作台、对话、连续性、AI 私域、能力、我的六个导航。后续元素不能算作 R8 已完成，不扩展助手市场或多助手协作，不减少原真实功能或安全墙要求。
...
- R2 个人所有者拥有一个 User Space，可真实创建、读取、修改、选择多个助手；切换不能改变设定、私域、消息、记忆、上下文或费用的既有归属。R3 已为各助手实现相互隔离的多个会话和独立当前选择，不扩展为助手市场或多助手协作。
- R3 迁移 `026` 将原 R1 默认会话原位登记为首个目录项，并新增分支/版本/附件/导出/operation 事实；原 MessageVersion 和 R1 execution/usage/result 不复制、不覆盖。精确合同见 [R3 多会话合同](../../backend/docs/R3_MULTI_CONVERSATION_CONTRACT.md)。
- R4 迁移 `027` 新增对话 Context 设置、turn 控制、assembly/source、结构化 summary/attempt 和 recovery action，并把唯一 snapshot hash 绑定到原 standalone execution；不复制 R1/R3 turn、Provider attempt、result、usage 或 MessageVersion。精确合同见 [R4 上下文装配合同](../../backend/docs/R4_CONTEXT_ASSEMBLY_CONTRACT.md)。
...
- R1 每个助手只登记一个默认独立 Conversation；Message 保存稳定顺序和当前版本指针，MessageVersion 保存不可覆盖的原始、编辑和重生成记录。R1 turn 只引用其锁定版本并支持连续多轮；R3 已在独立迁移中新增每助手多个会话、列表、重命名和受控删除，不改写 R1 事实。
...
- 历史 Context API 仍只读投影设定、状态记录、事件、近期消息和跨窗口摘要，其 Memory 字段继续保留原 `not_implemented` 语义；该端点不调用模型。R5 记忆只通过独立个人服务以精确 version/source hash 进入 R4 独立聊天的 `long_term_memory` 槽，不改写已锁定 turn；仅在既有 Engine 合同内，Vio 提供经权限筛选的事实，Engine 组织其最终认知 Context。
...
- 上一条只描述历史 Registry/准备 API；R6 新的个人能力 API 已形成真实、可恢复的本地 Tool/MCP/Skill/Plugin 执行事实，并通过统一账本与旧记录隔离。它不执行插件代码，不调用真实第三方 MCP，也不触碰外部 Subject Runtime。
...
- 真实第三方 MCP 服务验收、插件代码/市场、任意 Skill/Tool、窗口重置、私域披露/删除、外部存储、完整备份恢复、自动提醒、健康设备、真实语音/系统唤醒、后台调度、正式数据库、真实设备/机器人控制、载体迁移及其他厂商 API 仍待后续阶段。R6 只实现受限 MCP 与本地清单/编排边界；随机 loopback 不冒充真实服务。V4/S4-Live 已有一次真实 usage 事实，不代表完整费用管理或最终账单对账完成；支付/银行接入未实现，真实转账、代扣、账户冻结及助手市场/协作仍暂缓。
...
- [R6 统一能力执行合同](../../backend/docs/R6_UNIFIED_CAPABILITY_EXECUTION_CONTRACT.md)
