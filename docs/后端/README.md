...
R0-A 已冻结通用端口；R0-B 已默认装配 None Adapter，并提供 `GET /api/v1/subject-runtime/status` 与 `/health.subjectRuntime`；R0-C 已完成“我的”页真实状态读取及 StrictMode 生命周期修复。A/B/C 均已验收并推送，`none / disconnected / not_configured` 与平台 `available` 是合法状态。当前聊天仍沿用既有链路，未切换到该端口；状态卡不证明独立聊天已完成。R0 整体验收仍待总体协调窗口复核，R2、R1 尚未开始。

已确认但尚未实施的产品顺序：R0 验收后先 R2，再 R1，然后 R3 至 R13，编号不变。R2 完成真实登录和同一用户多个助手的创建、列表、选择与普通切换；R3 完成每个助手多个会话及隔离。R8 完成公共机制及截至 R7 已完成模块的前端接线；R9—R11 各自完成对应前后端真实联调，R13 全量复验；保留工作台、对话、连续性、AI 私域、能力、我的六个导航。后续元素不能算作 R8 已完成，不扩展助手市场或多助手协作，不减少原真实功能或安全墙要求。
...
- F1 对话页已读取固定本地 Profile 的真实 V5 Message/Turn 数据，R0-C 设置页已读取通用状态；其余未接线页面仍使用演示或 mock，真实登录与多助手产品流程尚未实现。
...
- 开发期数据层每个用户拥有一个 User Space，可保存助手列表与当前助手指针；一个 User Space 不等于每用户只能一个助手。现有基础不等于 R2 真实登录、多助手创建/选择/切换及 R3 各助手多会话已验收；切换不能改变设定、私域、消息、记忆、上下文或费用的既有归属。
...
- Model Router 的公共选择接口只返回本地模型描述；V4 另有受控 `openai_compatible` 执行、credential reference 与真实 usage 账本，已由首次 S4-Live 验证一次真实调用。完整凭据密钥库及真实身份配置仍属 R2，不把 Router 的未执行状态等同于整个 Vio 从未调用模型。
...
- F1 真实页面回复和首次真实供应商试聊已通过；Vio 费用账本为 `not_reported`，不把供应商概览的 ¥0 当作最终账单。尚未实现真实登录、多助手产品能力、独立聊天、通用 Binding、生产认证、多租户和部署。
