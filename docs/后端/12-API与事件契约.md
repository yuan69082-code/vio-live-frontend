...
- 当前状态：个人会话认证、原权限安全链及 R2/R1 个人接口已于 2026-09-05 正式验收通过，R3 于 2026-09-06 验收通过，R4、R5 与 R6 于 2026-09-08 验收通过。R6 已形成统一能力执行合同、迁移 `029`、个人 API、严格 MCP `2026-07-28` Streamable HTTP 客户端、受控本地 Tool、Skill 编排、Plugin 生命周期和 owner/current-assistant 隔离。公开认证/部署、R7 主动能力与 R9 设备执行仍未完成。

## 目标

建立前端、Vio Core、可选外部主体运行时、AI 模型和外部能力之间的稳定边界。接口变化不能依赖页面内部状态，也不能让前端直接持有数据库、密钥或设备厂商协议。现行顺序按 [ADR-034](../决策记录.md#adr-034)：R0、R2、R1、R3、R4、R5 与 R6 均已验收；R7 未开始。
...
通用职责：Vio 负责模型路由、自有 Context/记忆、Token、权限安全、真实执行和费用记录；R1 已在真实个人会话和当前助手范围内由 Vio 模型执行链形成普通回复。外部模式的候选、最终表达和受控投影仍须遵守已选适配器合同，模型服务不能越权修改数据、权限或外部 SubjectState；外部运行时选择与连接未在 R1 实施。
...
R6 将 `model_api`、`mcp_tool`、`local_tool`、`skill` 与 `plugin_action` 纳入同一不可变执行账本和个人 API。所有写操作只接受 R2 已验证会话和服务端当前助手，经过既有 Permission、Security、Confirmation 与 Audit 链；安装定义/版本、发现、依赖、生命周期、执行/历史与 by-key 恢复均按 `owner + current assistant` 隔离，同一 owner 的助手之间也不能共享事实。R2 Provider/Model 仍是 owner 级账户配置。读取目录、刷新、查询历史或按原幂等键恢复不会触发执行。R1 模型执行只投影既有事实，不产生第二次模型调用。MCP 只支持 `2026-07-28` Streamable HTTP，生产端点必须是通过 DNS 全结果及固定连接地址校验的公共 HTTPS；显式测试注入才允许随机 loopback HTTP。内置 Tool 只有确定性文本检查能力，不访问 shell、文件系统、进程环境或网络；Skill 仅编排受支持能力，Plugin 仅执行已登记 manifest action，不加载插件代码。

R6 公共路由位于 `/api/v1/personal/capabilities` 下，覆盖目录、安装/配置、MCP 发现、执行创建、单项/历史/幂等查询、显式恢复和 Plugin 启停/卸载。执行状态、稳定错误、确认、usage/cost、取消与未知结果恢复以 [`R6_UNIFIED_CAPABILITY_EXECUTION_CONTRACT.md`](../../backend/docs/R6_UNIFIED_CAPABILITY_EXECUTION_CONTRACT.md) 为唯一合同；外部 JSON 不返回执行输入、凭据、Authorization 或 MCP 原始响应。`outcome_unknown` 只能查询或等待正式对账；明确 `FAILED_RETRYABLE` 也必须由用户显式恢复并重新经过现实门控。R6 不包含 R7 主动执行、R9 设备控制、通用插件代码宿主或真实外部服务验收。
...
| 当前未完成 | R5 正式阶段验收、通用 Binding、HTTPS/远程部署、多租户；按已确认归属阶段实施 |
...
R1 已完成 Vio 自有独立聊天路径和旧固定身份生产入口隔离，不要求真实引擎参与。未经用户另行要求重连，不探测、读取、启动或修改真实 Engine；通用接口必须测试，真实引擎联调另算，不是 Vio 完成或发布条件。
...
历史 Context API 支持限制近期消息和跨窗口摘要数量，按系统安全位置、完整助手全局设定、当前状态、未解决事件、近期消息、跨窗口摘要、Memory 位置、本轮用户消息的顺序返回；其 Memory 位置继续固定 `not_implemented`，模型、外部 API 和 continuity-engine 固定 `not_performed`。R5 通过独立个人服务把 owner/assistant、权限安全和用户 Context 参与控制后的精确记忆版本接入 R4 独立 turn 快照；这不改写历史 Context API 合同。
