...
[`Continuity Integration Contract v1.1`](docs/后端/14-continuity-engine连接契约v1.1.md) 已由 Continuity Engine 正式接受。Engine E1/E2/E3 已在引擎提交 `c732f35` 完成，Vio V1 基线为 `c1e1336`；Vio V2 现已在本地实现严格 Engine success/error envelope 校验、不可覆盖结果账本、独立权威投影版本/回执/当前指针、revision 隔离和跨重启恢复。**这不表示 Vio 已连接 continuity-engine**：V2 只使用可注入的 test-only fixture transport，生产默认端口明确不可调用；双方共享测试、网络传输和生产 Integration Adapter 仍未开始。Continuity Engine 继续是 AI SubjectState 唯一权威源，前端也仍只能连接 Vio 后端。
...
- Vio V2 已实现与 Engine `c732f35` 精确一致的 success/error envelope 白名单校验、operation/response/stateProjection 幂等结果账本、独立投影版本与当前指针、revision 冲突隔离和崩溃恢复；首次合法 `changed=false / revision 0` 会初始化已确认投影 head，但不推进 revision，已有 head 时 `changed=false` 不改变指针；`changed=true` 只有通过 hash、唯一性与 CAS 校验后才推进一次。
- V2 仅提供进程内 test-only fixture transport；默认 transport 明确未配置，不存在 HTTP、Python 子进程或真实 Engine 调用。双方共享测试、网络连接和生产 Adapter 仍未实现。
