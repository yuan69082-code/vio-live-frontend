...
当前 Windows 个人本机运行可先双击 `首次准备 Vio.cmd`，以后用 `打开 Vio.cmd` / `停止 Vio.cmd` 日常启停（同时保留英文同义入口）。首次准备固定使用 Node 22.23.1 已验证基线和 Corepack 解析的 pnpm 11.9.0，以 `--offline --frozen-lockfile --force` 重建根依赖并完成类型检查/生产构建；缓存不足时停止，不在线回退。日常打开不安装、不升级、不构建，只有后端与静态前端都真实就绪后才打开浏览器；重复点击复用同一实例，停止只关闭能够证明由该入口启动的进程。

入口绑定当前仓库及现有 `backend/data/vio-live.dev.sqlite`，不会静默创建另一份空库或从其他 checkout 打开数据。服务只监听 `127.0.0.1`，不启动或探测 Engine。完整步骤、运行文件和故障处理见 [Windows 本机启停说明](docs/WINDOWS_LOCAL_START.md)。这不替代下方开发终端和安卓可信 LAN 验收入口，也不代表服务器部署或 R7 已完成。

第一次打开若页面显示“初始化邀请”，先停止 Vio，再双击 `初始化个人空间 Vio.cmd`（英文入口 `Initialize Personal Vio.cmd`）。该入口只接受首次准备写入的严格运行绑定，只操作已绑定的现有个人数据库，并把 15 分钟一次性邀请写入当前 Windows 用户的 `%LOCALAPPDATA%\VioLive\private`；邀请不会出现在终端、日志或网页查询接口。用户在初始化页面粘贴邀请，并自行设置 12—256 字符的个人访问口令；脚本不读取、生成或保存该口令。首次准备与个人初始化必须由实际桌面用户运行，施工工具隔离视图中的 AppData 文件不构成桌面可用证据。
