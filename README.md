...
后端现可按用户保存 Provider/Model 路由配置、四类扩展能力注册元数据和七类设备注册元数据。设备适配器只提供小米、美的、Apple、Android 与通用类型的未配置描述；设备操作准备只完成 Permission、Security 和 Confirmation 检查并保存 `not_executed` 日志。Tool 仍不执行，MCP 不连接，Plugin 不安装，Skill 不执行，设备不连接且不调用厂商 API。前端入口仍只通过 Vite 同源代理非阻塞检查 `/health`，页面尚未迁移到这些真实 API。
