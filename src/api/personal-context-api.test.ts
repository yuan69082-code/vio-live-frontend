...
const plan = {
  ...
  runtimeProjection: { status: 'not_available', sourceRef: null }, memory: { status: 'not_implemented' },
...
describe('R4 personal context API', () => {
  it('strictly parses a scoped plan, fixed slot order and deterministic budget', () => {
    expect(parseContextAssembly(plan, { conversationId: 'conversation-r4', branchId: 'branch-r4' })).toMatchObject({ mode: 'balanced', sources: [{ origin: 'cross_window' }], memory: { status: 'not_implemented' } })
